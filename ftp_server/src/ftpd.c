...
#endif
#if defined(F_FILESYSTEM)
    				ftp.fr = f_open(&(ftp.fil), (const char *)ftp.filename, "r");
    				//print_filedsc(&(ftp.fil));
    				if(ftp.fr == FR_OK){
    					remain_filesize = ftell(ftp.fil);
#if defined(_FTP_DEBUG_)
...
#endif
#if defined(F_FILESYSTEM)
    				ftp.fr = f_open(&(ftp.fil), (const char *)ftp.filename, "w+");
    				//print_filedsc(&(ftp.fil));
    				if(ftp.fr == FR_OK){
#if defined(_FTP_DEBUG_)
    					printf("f_open return FR_OK\r\n");
...
char proc_ftpd(char * buf)
{
	...
	switch (cmdp - commands)
	{
		...
		case XMKD_CMD:
			...
			arg[slen - 2] = 0x00;
#if defined(F_FILESYSTEM)
			if (f_mkdir(ftp.workingdir, arg) != 0)
			{
